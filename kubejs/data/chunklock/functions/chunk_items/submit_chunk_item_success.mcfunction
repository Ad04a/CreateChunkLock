...
execute if entity @s[nbt={Item: {id: "kubejs:token"}}] run scoreboard players set @e[type=marker,tag=currentChunk,tag=locked,limit=1] ChunklockChunkItemCount 0
execute if entity @s[nbt={Item: {id: "kubejs:token"}}] run scoreboard players set @s ChunklockChunkItemOverflowCount 0
...
execute if entity @s[nbt={Item: {id: "kubejs:token"}}] as @e[type=marker,tag=currentChunk,tag=locked,limit=1] if score @s ChunklockChunkItemCount matches ..0 at @s run function chunklock:chunk_items/unlock_nearest_chunks
execute unless entity @s[nbt={Item: {id: "kubejs:token"}}] as @e[type=marker,tag=currentChunk,tag=locked,limit=1] if score @s ChunklockChunkItemCount matches ..0 at @s run function chunklock:utils/unlock_current_chunk
